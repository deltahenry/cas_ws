...
class MotionController(Node):
    def __init__(self):
        ...
        self.debug_mode_subscriber = self.create_subscription(
            Bool,
            'debug_cmd',
            self.debug_mode_callback,
            10
        )
    ...
    def debug_mode_callback(self,msg: Bool):
        """接收來自 UI 的 debug mode 訊息"""
        self.debug_mode = msg.data
            ...
            if self.debug_mode:
                if msg.command_type == MotionCmd.TYPE_GOTO_RELATIVE:
                    ...
                    joint_pose_cmd =[
                        ...
                        self.current_motor_len[1] + (-msg.pose_data[0]),
                        self.current_motor_len[2] + (-msg.pose_data[0]),
                    ]
                    print(f"Relative joint pose: {joint_pose_cmd}")
                    self.move_j(joint_pose_cmd, msg.speed)
                    
                else:
                    self.get_logger().error("In debug mode, only relative joint movement is allowed.")
    ...
    def move_home(self):
        ...
        joint_target = self.robot_model.inverse_kinematics(home_pose)  # 使用機器人模型的逆運動學計算關節目標位置 -M3-M2-M1
    ...
    def move_j(self,joint_pos_cmd,speed):
        # 1. Inverse kinematics
        joint_target =  joint_pos_cmd         
        # 2. Interpolate trajectory
        current_joint_pos = self.current_motor_len
        trajectory = self.interpolate_joint_space(current_joint_pos, joint_target,vdes=10.0)  # 使用插值函數計算關節軌跡
        # 3. Add to trajectory queue
        self.load_trajectory(trajectory)  # 將軌跡載入 queue
    ...
    def send_next_batch(self):
        ...
        yaw = self.current_cartesian_pose[2]#rad
        pose_data = [x, y, yaw]
