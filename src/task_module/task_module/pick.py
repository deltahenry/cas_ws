...
class RoughAlignFSM(Machine):
    def __init__(self, data_node: DataNode):
        ...
        transitions = [
            ...
            {'trigger': 'check_depth_to_pick', 'source': RoughAlignState.CHECK_DEPTH.value, 'dest': RoughAlignState.pick.value},
            {'trigger': 'pick_to_done', 'source': RoughAlignState.pick.value, 'dest': RoughAlignState.DONE.value},
    ...
    def run(self):
        if self.state == RoughAlignState.IDLE.value:
            ...
            if self.data_node.depth_data[0] < depth_ref:
                ...
                self.check_depth_to_pick()
            else:
                print("depth_data 大於或等於參考深度，waiting for human push")
        
        elif self.state == RoughAlignState.pick.value:
